...
class PlanningGraph():
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        a1_neg_a2 = bool(set(node_a1.action.effect_add).intersection(set(node_a2.action.effect_rem)))
        a2_neg_a1 = bool(set(node_a1.action.effect_rem).intersection(set(node_a2.action.effect_add)))
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        a1_neg_a2 = bool(set(node_a1.action.effect_rem).intersection(set(node_a2.action.precond_pos)))
        a2_neg_a1 = bool(set(node_a1.action.precond_pos).intersection(set(node_a2.action.effect_rem)))
        return a1_neg_a2 or a2_neg_a1
    ...
    def h_levelsum(self) -> int:
        ...
        """
        level_sum = 0
        for goal_s in self.problem.goal:
            goal_found = False
            for level in range(len(self.s_levels)):
                for fluent in self.s_levels[level]:
                    if fluent.symbol == goal_s and fluent.is_pos:
                        goal_found = True
                        level_sum += level
                        break
                if goal_found:
